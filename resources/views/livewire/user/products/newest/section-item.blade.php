<div class="card-scroll-item">
    ...
    @if (!$product->deleted_at && !$product->special && !$product->has_offer && $product->is_newest)
        <div class="product-card border-success border-opacity-10 h-100">
            <!-- شارة "جديد" بلون أخضر -->
            <span class="badge-featured bg-success text-white">
                <i class="bi bi-lightning-fill me-1"></i>جديد
            </span>
            ...
            <a href={{ route('user.product', ['type' => 'newest', 'id' => $product->id]) }} class="card-img-wrapper">
            ...
            <div class="card-body">
                <a href={{ route('user.product', ['type' => 'newest', 'id' => $product->id]) }}
                    class="product-title">{{ $product->name }}</a>
                <div class="product-price text-success">{{ $product->price }}ل.س</div>
                @if (
                    $product->available &&
                        $product->quantity > 0 &&
                    ...
                    @if ($IsCartItem)
                        <button wire:click.prevent='removeFromCart' wire:loading.attr='disabled'
                            class="btn btn-success btn-add-cart opacity-75">
